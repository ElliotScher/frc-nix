...
fetch_github_hashes() {
    ...
    case "$tool" in
        ...
        "AdvantageScope")
            ...
            # Compute npmDepsHash and docs npmDepsHash from the source.
            # docs.nix uses unpatched source, but package.nix applies patches
            # that modify package-lock.json, so we compute docs hash first,
            # then apply patches and compute the main hash.
            local tmpdir
            tmpdir=$(mktemp -d)
            curl -sL "$url" | tar xz -C "$tmpdir" --strip-components=1
            # docs.nix uses raw (unpatched) source
            ...
            # package.nix applies patches that modify package-lock.json
